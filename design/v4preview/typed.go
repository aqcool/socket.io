...
// AckEmitter is the narrow low-level contract needed by typed EmitAck.
// A Socket should implement this directly; broadcast ACK collection may use a
// separate multi-response contract.
type AckEmitter interface {
	EmitAck(context.Context, string, ...any) ([]any, error)
}

// MultiAckEmitter is used by namespace/broadcast operations that collect one
// acknowledgement per matching client.
type MultiAckEmitter interface {
	EmitAcks(context.Context, string, ...any) ([][]any, error)
}

func Emit[Request, Response any](
	emitter RawEmitter,
