...
func TestParserv3(t *testing.T) {
	...
	t.Run("encodeOneBinaryPacket", func(t *testing.T) {
		...
		if b := data.Bytes(); !bytes.Equal(b, check) {
			t.Fatalf(`binary encodeOneBinaryPacket value not as expected: %v, want match for %v`, b, check)
		}
	})
	...
	t.Run("encodeOneBinaryPacket/String", func(t *testing.T) {
		...
		check := append([]byte{0x00, 0x05, 0x08, 0xFF, PACKET_TYPES[packet.OPEN]},
			[]byte("test测试中文和表情字符❤️🧡💛🧓🏾💟")...)
		if b := data.Bytes(); !bytes.Equal(b, check) {
	...
	t.Run("EncodePayload", func(t *testing.T) {
		...
		check := append([]byte{1, 4, 255, 0, 65, 66, 67, 0, 5, 8, 255, PACKET_TYPES[packet.CLOSE]},
			[]byte("test测试中文和表情字符❤️🧡💛🧓🏾💟")...)
		if b := data.Bytes(); !bytes.Equal(b, check) {
			t.Fatalf(`DecodePacket *Packet.Data value not as expected: %v, want match for %v`, b, check)
		}
	...
	t.Run("DecodePayload/Base64", func(t *testing.T) {
		...
		if l := len(packs); l != 2 {
			t.Fatalf(`base64 len(packs) = %d, want match for %d`, l, 2)
		}
	...
	t.Run("DecodePayload", func(t *testing.T) {
		...
		officialPayload, encodeErr := p.EncodePayload([]*packet.Packet{
			{Type: packet.OPEN, Data: bytes.NewBuffer([]byte("ABC"))},
			{Type: packet.CLOSE, Data: strings.NewReader("test测试中文和表情字符❤️🧡💛🧓🏾💟")},
		}, true)
		if encodeErr != nil {
			t.Fatal("Error with EncodePayload:", encodeErr)
		}
		packs, decodeErr := p.DecodePayload(officialPayload)
		if decodeErr != nil {
			t.Fatal("Error with DecodePayload:", decodeErr)
		}
		if l := len(packs); l != 2 {
