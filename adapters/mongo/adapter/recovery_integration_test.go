...
func TestMongoCrossNodeConnectionStateRecovery(t *testing.T) {
	...
	database := client.Database(fmt.Sprintf("socket_io_test_%d", time.Now().UnixNano()))
	defer func() { _ = database.Drop(t.Context()) }()
	...
	newServer := func() *socket.Server {
		...
		serverOptions.SetAdapter(&MongoAdapterBuilder{Mongo: mongoClient})
	...
	if !ok || offset == "" {
		t.Fatalf("expected MongoDB ObjectID offset, got %#v", baselineData)
	}
	...
	if recoveredOffset, ok := packetData[len(packetData)-1].(string); !ok || recoveredOffset == "" {
		t.Fatalf("expected recovered ObjectID offset, got %#v", packetData)
	}
