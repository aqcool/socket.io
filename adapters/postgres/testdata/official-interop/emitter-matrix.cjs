...
const {
  createAdapter: createBaselineAdapter,
} = require("@socket.io/postgres-adapter-emitter-baseline");
const {
  createAdapter: createCurrentAdapter,
} = require("@socket.io/postgres-adapter");
const { Emitter } = require("@socket.io/postgres-emitter");

const [goURL, postgresURI, channelPrefix, tableName] = process.argv.slice(2);
if (!goURL || !postgresURI || !channelPrefix || !tableName) {
  throw new Error(
    "usage: node emitter-matrix.cjs <go-url> <postgres-uri> <channel-prefix> <table-name>",
  );
}

const timeoutMs = 10_000;
const propagationDelayMs = 350;
...
async function runOfficialTwelve(
  emitter,
  customEmitter,
  goURL,
  nodeURL,
  ...
  try {
    ...
    await expectBoth(
      ...
      "emitter-namespace",
      () => {
        customEmitter.emit("emitter-namespace", "custom");
      },
    ...
    await expectOnly(
      nodeSocket,
      goSocket,
      "emitter-except-event",
      () => {
        emitter
          .except("emitter-excluded")
          .emit("emitter-except-event", "except");
      },
...
async function runGoEmitterSmoke(goURL, nodeURL, nodeServer) {
  ...
  try {
    ...
    await expectBoth(
      ...
      () => {
        return command("broadcast", "go-emitter-broadcast", "go");
      },
      ([label, binary]) => {
    ...
    await expectBoth(
      ...
      () => {
        return command("namespace", "go-emitter-namespace", "custom");
      },
    );
...
async function startNodePeer(adapterFactory) {
  const pool = new Pool({ connectionString: postgresURI });
  const httpServer = createServer();
  const io = new Server(httpServer, { transports: ["websocket"] });
  io.adapter(adapterFactory(pool, { channelPrefix, tableName }));
  io.of("/custom");
...
async function stopNodePeer(peer) {
  if (!peer) return;
...
async function main() {
  ...
  const emitterOptions = { channelPrefix, tableName };
  const emitter = new Emitter(pool, "/", emitterOptions);
  // In 0.1.1, Emitter#of() does not carry custom options to the child emitter.
  // Constructing the namespaced emitter explicitly exercises the same official
  // wire protocol while retaining this isolated test channel and table.
  const customEmitter = new Emitter(pool, "/custom", emitterOptions);
  let peer;
  try {
    peer = await startNodePeer(createBaselineAdapter);
    await delay(propagationDelayMs);
    await runOfficialTwelve(emitter, customEmitter, goURL, peer.url, peer.io);
    await stopNodePeer(peer);
    peer = null;
    peer = await startNodePeer(createCurrentAdapter);
    await delay(propagationDelayMs);
    await runGoEmitterSmoke(goURL, peer.url, peer.io);
    process.stdout.write(
      `${JSON.stringify({
        emitter: "@socket.io/postgres-emitter@0.1.1",
        officialEmitterTestAdapter: "@socket.io/postgres-adapter@0.1.1",
        currentAdapter: "@socket.io/postgres-adapter@0.5.0",
        officialBehaviorMatrix: "12/12",
        officialEmitterToGoAdapter: "pass",
        goEmitterToCurrentOfficialAdapter: "pass",
        externalServiceGate: "SOCKET_IO_POSTGRES_OFFICIAL_INTEROP=1",
      })}\n`,
    );
  } finally {
    await stopNodePeer(peer);
